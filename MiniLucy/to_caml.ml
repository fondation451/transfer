...
let instr_to_caml instance i =
  ...
  let rec loop i =
    ...
    |IOBJ_step(id_l, id, e_l) ->
      ...
      let out = sprintf "let ____th%d____ = Thread.create (fun () -> " !n in
    ...
    |IOBJ_case(id, case_l) ->
      ...
      List.iter (fun (id_case, i1) -> out := !out ^ (sprintf "|%s -> %s" id_case (loop i1))) case_l;
      !out ^ "\n|_ -> ())) () in\n"
